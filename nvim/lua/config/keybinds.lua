...
local function toggle_terminal(direction)
    -- Find if there is a window displaying a terminal buffer in the current tabpage
    local term_win = nil
    ...
    if term_win then
        ...
        -- Look for an existing running terminal buffer that is not visible in any window
        local term_buf = nil
        for _, buf in ipairs(vim.api.nvim_list_bufs()) do
            if vim.bo[buf].buftype == "terminal" then
                local job_id = vim.b[buf].terminal_job_id
                if job_id and vim.fn.jobwait({job_id}, 0)[1] == -2 then
                    -- Check if this buffer is currently visible in any window
                    local visible = false
        ...
        if term_buf and vim.api.nvim_buf_is_valid(term_buf) then
            ...
            if direction == "bo" then
                vim.cmd("botright sbuf " .. term_buf)
            else
                vim.cmd("vert sbuf " .. term_buf)
            end
        else
